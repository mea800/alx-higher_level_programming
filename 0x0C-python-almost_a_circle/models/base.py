...
class Base:
    """Base class that provides common functionality for other classes.
    ...
    def create(cls, **dictionary):
        ...
        Create an instance of Rectangle or Square instance
        swith “dummy” mandatory,
        attributes (width, height, size, etc.),
        Call update instance method on this “dummy” instance to apply your,
        real values.
    ...
    def load_from_file(cls):
        ...
        # Create instances from dictionaries and append to list
    ...
    def save_to_file_csv(cls, list_objs):
        ...
        format and saves to a file.

        Args:
            cls (any): class.
            list_objs (list): objects.
        """
        # Define filename based on class name
        filename = cls.__name__ + ".csv"
        with open(filename, 'w', newline="") as f:
            writer = csv.writer(f)
            if cls.__name__ == "Rectangle":
                # Write attributes of Rectangle objects as rows in CSV file
                for i in list_objs:
                    writer.writerow([i.id, i.width, i.height, i.x, i.y])
            elif cls.__name__ == "Square":
                # Write attributes of Square objects as rows in CSV file
                for i in list_objs:
                    writer.writerow([i.id, i.size, i.x, i.y])

    @classmethod
    def load_from_file_csv(cls):
        """Deserializes a list of rectangles or squares from a
        CSV file and returns a list of instances.
        ...
        try:
            with open(filename, 'r') as f:
                ...
                for elm in csv_reader:
                    if cls.__name__ == "Rectangle":
                        # Create dictionary with attribute values for Rectangle
                        dictionary = {"id": int(elm[0]), "width": int(elm[1]),
                                      "height": int(elm[2]), "x": int(elm[3]),
                                      "y": int(elm[4])}
                    elif cls.__name__ == "Square":
                        # Create dictionary with attribute values for Square
                        dictionary = {"id": int(elm[0]), "size": int(elm[1]),
                                      "x": int(elm[2]), "y": int(elm[3])}
                    # Create instance from dictionary and append to list
    ...
    Opens a window and draws all the Rectangles and Squares on it.

        """
        # Create turtle graphics windows
